...
const SubCategory = () => {
  ...
  const getItems = async () => {
    try {
      setstate("get");
      setitem(null);
      const myitems = await axios.get("http://localhost:5000/subcategories");
  ...
  const saveItem = async (e) => {
    try {
      setstate("create");
      e.preventDefault();
      await axios.post("http://localhost:5000/subcategories", item);
      getItems();
    } catch (error) {
  ...
  const deleteItem = async () => {
    try {
      setstate("delete");
      await axios.delete("http://localhost:5000/subcategories/" + item.id);
      getItems();
      setitem(null);
    } catch (error) {
  ...
  const updateItem = async () => {
    try {
      ...
      delete item.id;
      await axios.patch("http://localhost:5000/subcategories/" + itemId, item);
      getItems();
      setitem(null);
    } catch (error) {
      console.log(error, "<<< deleteItem()")
